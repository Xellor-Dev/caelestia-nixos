{lib, upstream, ...}: let
  ...
  upstreamConf =
    if upstream != null && builtins.pathExists "${upstream}/hypr/hyprland/animations.conf"
    then parser.parseSections (builtins.readFile "${upstream}/hypr/hyprland/animations.conf")
    else lib.warn "caelestia-nixos: upstream hypr/hyprland/animations.conf not found, using fallback defaults" {
      animations = {
        enabled = "true";
        bezier = [
