...
```bash
# Build the Home Manager module (without switching)
# Create a temporary test configuration
mkdir -p /tmp/caelestia-test
cat > /tmp/caelestia-test/flake.nix << 'EOF'
{
  inputs = {
    nixpkgs.url = "github:NixOS/nixpkgs/nixos-unstable";
    home-manager = {
    ...
    caelestianix = {
      path = /path/to/caelestia-nixos;  # Point to your local repo
      inputs.nixpkgs.follows = "nixpkgs";
    };
  };
...
cat > /tmp/caelestia-test/home.nix << 'EOF'
{ config, pkgs, ... }:

{
  programs.caelestia-dots = {
    enable = true;
    hypr.enable = true;      # Test Hyprland module
    term.enable = true;      # Test terminal module
...
```bash
# Test only Hyprland parsing
nix eval --impure --expr '
let
  flake = builtins.getFlake (toString /path/to/caelestia-nixos);
  pkgs = import flake.inputs.nixpkgs { system = "x86_64-linux"; };
  lib = pkgs.lib;
  upstream = "\${flake.inputs.caelestia-dots}";
  
  # Test the hyprconf parser
  parser = import ./configs/hypr/hyprconf-parser.nix {inherit lib;};
  rawConf = builtins.readFile "\${upstream}/hypr/hyprland/general.conf";
  parsed = parser.parseSections rawConf;
in
  {
    keysFound = builtins.attrNames parsed;
    general = parsed.general or null;
  }
'
```
...
# 3. Build without switching (safe)
home-manager build --flake .#youruser

# 4. If successful, then switch
...
```bash
# See what files would be generated without applying them
home-manager build --flake .#youruser
ls -la $(nix eval --raw .#homeConfigurations.youruser.activationPackage)/home-files/
```

### Debug a specific module

```bash
# Check if upstream paths are found
nix eval --impure --expr '
  let
    flake = builtins.getFlake (toString /path/to/caelestia-nixos);
    upstream = "\${flake.inputs.caelestia-dots}";
  in
    builtins.pathExists "\${upstream}/hypr/hyprland/general.conf"
'
...
```bash
# Test the complete pipeline: parse → substitute → merge → generate
home-manager build --flake .#youruser

# Check that generated config is valid
hyprctl --instance 0 reload  # If you have Hyprland running

# Or just check syntax without reloading
hyprctl instances  # See if Hyprland recognizes the new config
```
